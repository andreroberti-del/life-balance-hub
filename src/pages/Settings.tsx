...
function SettingLink({ label, description, external }: SettingLinkProps) {
  return (
    <button className="w-full flex items-center justify-between py-4 border-b border-border last:border-0 hover:bg-dark3 transition-colors text-left rounded-lg">
      ...
      {external ? (
        <ExternalLink className="w-4 h-4 text-text3 flex-shrink-0" />
      ) : (
        <ChevronRight className="w-4 h-4 text-text3 flex-shrink-0" />
      )}
    </button>
  );
}
...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <h2 className="text-2xl font-bold text-text">Configuracoes</h2>
        <p className="text-text2 mt-1">Personalize sua experiencia</p>
      </div>
      ...
      <div className="bg-card rounded-2xl p-6 border border-border">
        ...
        <div className="py-4">
          <p className="text-sm font-medium text-text mb-3">Unidades</p>
          <div className="flex gap-3">
