    ...
    <div className="min-h-screen p-6 md:p-8 bg-violet-50">
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-violet-500 rounded-3xl p-8 shadow-lg">
            <p className="text-sm font-bold text-white/80 mb-2 uppercase tracking-wide">{t.community.improvedRatio}</p>
            <div className="flex items-baseline gap-2 mb-2"><span className="text-5xl font-bold text-white">12:1</span><span className="text-3xl text-white/60">→</span><span className="text-5xl font-bold text-white">4.2:1</span></div>
            <p className="text-xs text-white/70">{t.community.communityAverage}</p>
          </div>
            ...
            <div className="bg-white rounded-3xl p-6 border border-gray-200/50 shadow-sm">
              ...
              <div className="space-y-3">
                {weeklyLeaderboard.map((user) => (
                  <div key={user.rank} className={`p-5 rounded-2xl border-2 transition-all ${user.isCurrentUser ? 'bg-violet-500/10 border-violet-500' : 'border-gray-100 hover:border-gray-300'}`}>
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl font-bold ${user.rank === 1 ? 'bg-violet-500 text-white' : user.rank === 2 ? 'bg-gray-300 text-white' : user.rank === 3 ? 'bg-violet-500 text-white' : 'bg-gray-100 text-gray-600'}`}>{user.rank}</div>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {user.isCurrentUser && <span className="px-2 py-0.5 bg-violet-500 text-white rounded-full text-xs font-bold">{t.community.you}</span>}
                        </div>
                      ...
                      <div className={`px-4 py-2 rounded-xl text-xs font-bold ${user.level === 'leader' ? 'bg-violet-500 text-white' : user.level === 'ambassador' ? 'bg-violet-500 text-white' : 'bg-gray-100 text-gray-700'}`}>{t.community[user.level as keyof typeof t.community]}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white rounded-3xl p-6 border border-gray-200/50 shadow-sm">
              ...
              <div className="space-y-4">
                {protocolGroups.map((group, i) => (
                  <div key={i} className={`p-5 rounded-2xl border-2 ${group.active ? 'bg-violet-500/10 border-violet-500/40' : 'border-gray-100'}`}>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      {group.active && <span className="px-3 py-1.5 bg-violet-500 text-white rounded-xl text-xs font-bold">{t.community.active}</span>}
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-600 font-semibold">{group.members} {t.common.members}</span>
                      <div className="flex items-center gap-3"><span className="text-sm font-bold text-violet-950">{group.avgProgress}%</span><div className="w-32 bg-gray-100 rounded-full h-2.5 overflow-hidden"><div className="bg-violet-500 h-2.5 rounded-full" style={{ width: `${group.avgProgress}%` }}></div></div></div>
                    </div>
                  </div>
                ))}
              </div>
                ...
                {participationLevels.map((level, i) => (
                  <div key={i} className="p-4 rounded-2xl border border-gray-100 hover:border-black/20 transition-all">
                    <div className="flex items-start gap-3 mb-3">
                      <div className="w-10 h-10 rounded-xl bg-violet-500 flex items-center justify-center flex-shrink-0"><level.icon className="w-5 h-5 text-white" /></div>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-gray-100"><span className="text-xs text-gray-600 font-semibold">{t.common.members}</span><span className="text-sm font-bold text-violet-950">{level.members.toLocaleString()}</span></div>
                  </div>
                ))}
              </div>
            </div>
            <div className="bg-violet-500 rounded-3xl p-6 shadow-lg">
              <h3 className="text-xl font-bold text-white mb-5">{t.community.yourCommunityImpact}</h3>
              <div className="space-y-4">
                <div className="flex justify-between items-center"><span className="text-sm text-white/80 font-semibold">{t.community.currentLevel}</span><span className="text-lg font-bold text-white">{t.community.leader}</span></div>
                <div className="flex justify-between items-center"><span className="text-sm text-white/80 font-semibold">{t.community.rankInCohort}</span><span className="text-lg font-bold text-white">#3 of 347</span></div>
                <div className="flex justify-between items-center"><span className="text-sm text-white/80 font-semibold">{t.community.peopleInspired}</span><span className="text-lg font-bold text-white">12</span></div>
              </div>
            </div>
          </div>
        </div>
